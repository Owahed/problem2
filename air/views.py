...
#mine
from content.models import AirlinePolicy 
...
def book_review(request, flight):
    ...
    # seatmaps = Seatmaps(client=amadeus)
    print(request.GET)
   

    flight_order_id = flight['id']
    print("===========================================================================================")
    # print(flight_order_id)
    # seat_map_response = amadeus.shopping.seatmaps.get(flightOrderId=flight_order_id)

    # Convert the flight_order_id (which is an integer) to a string
    flight_order_id_str = str(flight_order_id)

    # Encode the flight_order_id_str to Base64
    flight_order_id_base64 = base64.b64encode(flight_order_id_str.encode('utf-8')).decode('utf-8')

    # print("===========================================================================================")
    print('flight order id : ')
    print(flight_order_id_base64)

    # Now, you can pass flight_order_id_base64 as flightOrderId in the seatmap API call
    seat_map_response = amadeus.shopping.seatmaps.get(flightOrderId=flight_order_id_base64)
    ...
    print('response :')
    print(seat_map_response)
    # Fetch the seat map data from the Amadeus API
    seat_map_data = seat_map_response.data
    print('----------------------------------------------------------------------------------------------')
    print(seat_map_data)
    ...
    return render(request, 'air/book_review.html', {
        ...
        "booking_policy": airline_policy.content,
        "roky": roky
    })
...
def test_book_travelers(request, flight):
    ...
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():
                ...
                # flight_offer=FlightOffer.objects.get(id=1)
                
                travelers = Traveler.objects.create(
